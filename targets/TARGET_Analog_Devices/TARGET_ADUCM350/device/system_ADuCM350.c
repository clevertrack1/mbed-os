...
static bool                     isLfClkUsed                     (void);

#ifdef ADI_SYSTEM_CLOCK_TRANSITION
static void                     changeClockDividers             (ADI_SYS_CLOCK_STATE_CONFIG_TYPE* pClockStateConfig, bool downshiftOnly);
...
/*----------------------------------------------------------------------------
  Static variables
 *---------------------------------------------------------------------------*/
static volatile bool   privateIntOccFlag = false;
...
*/
ADI_SYS_RESULT_TYPE SystemEnableClock (ADI_SYS_CLOCK_GATE_ID id, bool bEnable)
{
    uint16_t mask;
 ...
 *
 */
ADI_SYS_RESULT_TYPE SystemEnableClockSource (ADI_SYS_CLOCK_SOURCE_ID id, bool bEnable) {
    uint16_t                val;
...
ADI_SYS_RESULT_TYPE SystemClockSourcePowerDownIfUnused (ADI_SYS_CLOCK_SOURCE_ID id) {
    ...
    switch (id) {
        ...
        case ADI_SYS_CLOCK_SOURCE_HFOSC:
            ...
            if (((pADI_GPT0->CTL & BITM_GPT_GPTCON_CLK) == ADI_GPT_CLOCK_SELECT_HFOSC) &&
                (pADI_GPT0->CTL & BITM_GPT_GPTCON_ENABLE)) {
                return ADI_SYS_ERR_CLOCK_SOURCE_USED_GPT_CLK;
            }

            /* Check if HFOSC is used as clock input for GPT1 and GPT1 is enabled */
            if (((pADI_GPT1->CTL & BITM_GPT_GPTCON_CLK) == ADI_GPT_CLOCK_SELECT_HFOSC) &&
                (pADI_GPT1->CTL & BITM_GPT_GPTCON_ENABLE)) {
                return ADI_SYS_ERR_CLOCK_SOURCE_USED_GPT_CLK;
            }

            /* Check if HFOSC is used as clock input for GPT2 and GPT2 is enabled */
            if (((pADI_GPT2->CTL & BITM_GPT_GPTCON_CLK) == ADI_GPT_CLOCK_SELECT_HFOSC) &&
                (pADI_GPT2->CTL & BITM_GPT_GPTCON_ENABLE)) {
                return ADI_SYS_ERR_CLOCK_SOURCE_USED_GPT_CLK;
            }
        ...
        case ADI_SYS_CLOCK_SOURCE_LFXTAL:
            ...
            if (((pADI_GPT0->CTL & BITM_GPT_GPTCON_CLK) == ADI_GPT_CLOCK_SELECT_EXTERNAL_CLOCK) &&
                (pADI_GPT0->CTL & BITM_GPT_GPTCON_ENABLE)) {
                return ADI_SYS_ERR_CLOCK_SOURCE_USED_GPT_CLK;
            }

            /* Check if LFXTAL is used as clock input for GPT1 and GPT1 is enabled */
            if (((pADI_GPT1->CTL & BITM_GPT_GPTCON_CLK) == ADI_GPT_CLOCK_SELECT_EXTERNAL_CLOCK) &&
                (pADI_GPT1->CTL & BITM_GPT_GPTCON_ENABLE)) {
                return ADI_SYS_ERR_CLOCK_SOURCE_USED_GPT_CLK;
            }

            /* Check if LFXTAL is used as clock input for GPT2 and GPT2 is enabled */
            if (((pADI_GPT2->CTL & BITM_GPT_GPTCON_CLK) == ADI_GPT_CLOCK_SELECT_EXTERNAL_CLOCK) &&
                (pADI_GPT2->CTL & BITM_GPT_GPTCON_ENABLE)) {
                return ADI_SYS_ERR_CLOCK_SOURCE_USED_GPT_CLK;
            }
        ...
        case ADI_SYS_CLOCK_SOURCE_LFOSC:
            ...
            if (((pADI_GPT0->CTL & BITM_GPT_GPTCON_CLK) == ADI_GPT_CLOCK_SELECT_32KHZ_INTERNAL_CLOCK) &&
                (pADI_GPT0->CTL & BITM_GPT_GPTCON_ENABLE)) {
                return ADI_SYS_ERR_CLOCK_SOURCE_USED_GPT_CLK;
            }

            /* Check if LFOSC is used as clock input for GPT1 and GPT1 is enabled */
            if (((pADI_GPT1->CTL & BITM_GPT_GPTCON_CLK) == ADI_GPT_CLOCK_SELECT_32KHZ_INTERNAL_CLOCK) &&
                (pADI_GPT1->CTL & BITM_GPT_GPTCON_ENABLE)) {
                return ADI_SYS_ERR_CLOCK_SOURCE_USED_GPT_CLK;
            }

            /* Check if LFOSC is used as clock input for GPT2 and GPT2 is enabled */
            if (((pADI_GPT2->CTL & BITM_GPT_GPTCON_CLK) == ADI_GPT_CLOCK_SELECT_32KHZ_INTERNAL_CLOCK) &&
                (pADI_GPT2->CTL & BITM_GPT_GPTCON_ENABLE)) {
                return ADI_SYS_ERR_CLOCK_SOURCE_USED_GPT_CLK;
            }
...
/*!
    ...
    @param[in,out] pbInterruptOccurred
                                   ...
                                   to bool for software looping sleep between interrupts.
                                   ...
                                   shared bool application control veriable allowing the wake-up
 ...
 */
void SystemExitLowPowerMode(bool volatile * pbInterruptOccurred) {
...
static bool isLfClkUsed(void) {
    ...
    if ((pADI_CT->CFG2 & BITM_CT_CT_CFG2_STIMER_EN) &&
        (pADI_CT->CDC_PWR & BITM_CT_CT_CDC_PWR_PWR_MODE)) {
